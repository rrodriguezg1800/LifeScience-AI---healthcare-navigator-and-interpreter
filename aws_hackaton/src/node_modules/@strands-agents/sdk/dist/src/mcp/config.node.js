...
function baseOptions(name, server, defaults) {
    ...
    if (server.prefix !== undefined)
        opts.prefix = interpolateEnv(server.prefix);
    if (server.toolFilters !== undefined)
        opts.toolFilters = compileToolFilters(name, server.toolFilters);
    return opts;
}
function compileToolFilters(name, filters) {
    const compiled = {};
...
function compileFilterMatchers(name, key, patterns) {
