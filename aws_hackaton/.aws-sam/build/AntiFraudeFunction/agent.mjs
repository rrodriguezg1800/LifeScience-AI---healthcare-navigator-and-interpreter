...
import { Agent, BedrockModel, tool, ImageBlock, TextBlock } from "@strands-agents/sdk";
import { z } from "zod";
...
export async function* analyzeContent(text, imageBase64 = null, mimeType = "image/jpeg") {
  ...
  if (imageBase64) {
    // El SDK de Strands usa ImageBlock con format = subtipo sin "image/"
    // y source.bytes como Uint8Array (no base64 string).
    const format = mimeType.split("/")[1] ?? "jpeg"; // "image/png" → "png"
    ...
    userMessage = [textBlock, imageBlock];
  } else {
