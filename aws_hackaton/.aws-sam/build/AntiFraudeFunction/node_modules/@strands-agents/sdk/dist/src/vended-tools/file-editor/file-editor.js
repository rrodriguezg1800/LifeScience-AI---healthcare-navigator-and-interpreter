 ...
 * ```
 */
export const DEFAULT_FILE_EDITOR_DESCRIPTION = 'Filesystem editor tool for viewing, creating, and editing files. Supports view (with line ranges), create, str_replace, and insert operations. Files must use absolute paths.';
export function makeFileEditor(sandboxOrOptions, maybeOptions) {
    ...
    return tool({
        ...
        callback: async (input, context) => {
            if (!context)
                throw new Error('Tool context is required for fileEditor operations');
...
function validatePath(filePath) {
    ...
    if (!path.isAbsolute(filePath)) {
        ...
        throw new Error(`The path ${filePath} is not an absolute path, it should start with \`/\`. Maybe you meant ${suggestedPath}?`);
    }
...
function buildStrReplaceResult(originalContent, oldStr, newStr, filePath) {
    ...
    if (occurrences > 1) {
        ...
        const lineNumbers = lines
            ...
            .filter((num) => num !== -1);
        throw new Error(`No replacement was performed. Multiple occurrences of old_str \`${oldStr}\` in lines ${JSON.stringify(lineNumbers)}. Please ensure it is unique`);
    }
 ...
 */
function buildInsertResult(originalContent, insertLine, newStr) {
    ...
    if (insertLine < 0 || insertLine > nLines) {
        throw new Error(`Invalid \`insert_line\` parameter: ${insertLine}. It should be within the range of lines of the file: [0, ${nLines}]`);
    }
    const newStrLines = expandedNewStr.split('\n');
    const newFileTextLines = fileText === ''
        ? newStrLines
 ...
 */
function makeOutput(fileContent, fileDescriptor, initLine = 1) {
 ...
 */
async function probeSandboxPath(sandbox, filePath) {
 ...
 */
function assertWithinSizeLimit(content, maxSize = DEFAULT_MAX_FILE_SIZE) {
 ...
 */
async function listDirectory(sandbox, dirPath) {
    const items = [];
...
async function handleView(sandbox, filePath, viewRange) {
...
async function handleCreate(sandbox, filePath, fileText) {
...
async function handleStrReplace(sandbox, filePath, oldStr, newStr) {
    if (oldStr === undefined) {
...
async function handleInsert(sandbox, filePath, insertLine, newStr) {
