...
export class Swarm {
    ...
    async *_stream(input, invocationState, externalCancelSignal) {
        ...
        let caughtError;
        let result;
        let repetitiveHandoffReason;
        const invocationNodeHistory = [];
     ...
     */
    _validateRepetitiveHandoffConfig() {
        ...
        if ((window === 0) !== (minimumUniqueAgents === 0)) {
            throw new Error(`repetitive_handoff_detection_window=<${window}>, repetitive_handoff_min_unique_agents=<${minimumUniqueAgents}> | must both be 0 or both be positive`);
        }
        if (minimumUniqueAgents > window) {
            throw new Error(`repetitive_handoff_min_unique_agents=<${minimumUniqueAgents}>, repetitive_handoff_detection_window=<${window}> | minimum unique agents cannot exceed detection window`);
        }
        if (minimumUniqueAgents > this.nodes.size) {
            throw new Error(`repetitive_handoff_min_unique_agents=<${minimumUniqueAgents}>, nodes=<${this.nodes.size}> | minimum unique agents cannot exceed node count`);
        }
    }
     ...
     */
    _checkRepetitiveHandoff(nodeHistory) {
        const window = this.config.repetitiveHandoffDetectionWindow;
        if (window <= 0 || nodeHistory.length < window)
            return undefined;
