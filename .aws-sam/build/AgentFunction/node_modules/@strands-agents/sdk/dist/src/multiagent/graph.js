...
export class Graph {
    ...
    async *_stream(input, invocationState, externalCancelSignal) {
        ...
        let contentInput;
        if (resumeResponses) {
            contentInput = state._pendingInput;
        }
        else {
            contentInput = input;
        ...
        const targets = interruptResponsesByNode
            ? [...interruptResponsesByNode.keys()].map((id) => {
                ...
                if (!node) {
                    throw new Error(`node_id=<${id}>, graph_id=<${this.id}> | resume response targets a node missing from the graph; topology changed between save and resume?`);
                }
                return node;
            })
        ...
        try {
            while (targets.length > 0 || streams.size > 0) {
                ...
                while (!interrupted && targets.length > 0 && streams.size < this.config.maxConcurrency) {
                    ...
                    const nodeInput = this._resolveInputForScheduling(node, interruptResponsesByNode?.get(node.id), contentInput, state);
                ...
                while (queue.size > 0) {
                    ...
                    if (ready.length > 0) {
                        yield* this._emit(new MultiAgentHandoffEvent({
                            source: node.id,
     ...
     */
    async *_runBeforeNodeCall(node, nodeSpan, state, invocationState) {
     ...
     */
    async _streamNode(node, input, state, queue, nodeSpan, invocationState, executionSignal) {
        ...
        const nodeTimeoutHandle = nodeTimeoutController
            ...
            : undefined;
        const signals = [executionSignal, nodeTimeoutController?.signal].filter((s) => s !== undefined);
        const cancelSignal = signals.length > 0 ? AbortSignal.any(signals) : undefined;
        try {
            const gen = this._tracer.withSpanContext(nodeSpan, () => node.stream(input, state, {
                invocationState,
            ...
            if (nodeTimeoutController?.signal.aborted) {
                throw new Error(`node_timeout=<${nodeTimeout}>, node_id=<${node.id}>, graph_id=<${this.id}> | node exceeded wall-clock budget`);
            }
            ...
            if (nodeTimeoutHandle !== undefined)
     ...
     */
    _computeCanRunConcurrently() {
        if (this.config.maxConcurrency < 2)
            return false;
        if (this._sources.length > 1)
            return true;
        const fanOut = new Map();
        for (const e of this.edges)
            fanOut.set(e.source.id, (fanOut.get(e.source.id) ?? 0) + 1);
     ...
     */
    _resolveInputForScheduling(node, routed, contentInput, state) {
        if (routed) {
            const nodeState = state.node(node.id);
            if (!nodeState) {
                throw new Error(`node_id=<${node.id}>, graph_id=<${this.id}> | routed interrupt response targets a node missing from state; topology changed between save and resume?`);
            }
    ...
    _resolveNodeInput(node, input, state) {
        ...
        const blocks = typeof input === 'string'
            ? [new TextBlock(input)]
            : input.map((b) => ('type' in b ? b : contentBlockFromData(b)));
        return [...blocks, ...deps];
    }
    /**
     ...
     */
    async _findResumeTargets(state) {
        ...
        for (const [id, node] of this.nodes) {
            if (state.node(id)?.status === Status.COMPLETED)
                continue;
        ...
        if (ready.length > 0) {
            logger.debug(`resume_targets=<${ready.map((n) => n.id).join(', ')}>, prior_steps=<${state.steps}> | resuming graph from restored state`);
            return ready;
        }
     ...
     */
    async _allDependenciesSatisfied(incoming, state) {
        for (const edge of incoming) {
            if (state.node(edge.source.id)?.status !== Status.COMPLETED)
                return false;
