...
export class MemoryManager {
     ...
     */
    async _provideMemoryContext(messages, config) {
        ...
        const end = (injected, entryCount = 0, formatError = false) => {
     ...
     */
    _resolveInjectionQuery(messages, config) {
        if (config.query) {
            try {
     ...
     */
    _defaultInjectionFormat(entries) {
        const items = entries.map((entry) => entry.storeName
        ...
        try {
            targetStores =
                options?.stores !== undefined
                    ...
                    : this._config.stores;
            settled = await Promise.allSettled(targetStores.map((store) => store.search(query, {
     ...
     * @internal
     */
    async add(content, options, detached = false) {
        ...
        if (failures.length > 0) {
            const aggregateError = new AggregateError(failures.map((failure) => failure.reason), `MemoryManager: store writes failed: ${failures.map((failure) => failure.store).join(', ')}`);
