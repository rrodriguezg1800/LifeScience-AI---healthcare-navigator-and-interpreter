...
export class Node {
    ...
    async *stream(input, state, options) {
        ...
        const resolvedOptions = { ...options, invocationState };
        let result;
        try {
...
export class AgentNode extends Node {
    ...
    constructor(options) {
        ...
        if (preserveContext && !(agent instanceof Agent)) {
            throw new Error(`node_id=<${agent.id}> | preserveContext=true requires an Agent instance; non-Agent InvokableAgents cannot be snapshotted`);
        }
     ...
     */
    async *handle(input, state, options) {
        ...
        const preRunSnapshot = !this.preserveContext && isAgent ? this._agent.takeSnapshot({ preset: 'session' }) : undefined;
        ...
        const agentInternals = options?.bufferOutput && isAgent ? this._agent : undefined;
        ...
        try {
            ...
            if (agentInternals && originalPrinter) {
                ...
                if (buffered.length > 0)
                    originalPrinter.write(buffered);
            }
        }
    }
}
