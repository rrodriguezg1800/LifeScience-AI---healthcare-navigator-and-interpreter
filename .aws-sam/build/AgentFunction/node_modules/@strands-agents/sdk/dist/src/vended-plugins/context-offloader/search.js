...
function truncate(output, maxChars, message) {
...
function formatLines(lines, indices, matchedSet) {
    if (indices.length === 0)
        return '';
...
function searchByPattern(lines, pattern, scopeStart, scopeEnd, contextLines, maxChars, scopeLabel) {
    let regex;
    ...
    const body = formatLines(lines, [...visible].sort((a, b) => a - b), matchedSet);
...
function searchByLineRange(lines, start, end, totalLines, maxChars) {
...
export function isSearchableContent(contentType) {
 ...
 */
export function searchContent(text, input, maxChars) {
