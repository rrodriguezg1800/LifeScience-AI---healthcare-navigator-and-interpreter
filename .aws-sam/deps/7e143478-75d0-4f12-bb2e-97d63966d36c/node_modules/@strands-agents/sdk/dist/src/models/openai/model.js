...
function buildMantleClient(bedrockMantleConfig, apiKey, clientConfig, modelId) {
    if (apiKey !== undefined) {
        throw new Error("'apiKey' cannot be combined with 'bedrockMantleConfig'; the API key is derived from the Mantle config automatically.");
    }
    const conflicting = [];
    if (clientConfig?.apiKey !== undefined)
        conflicting.push('apiKey');
    if (clientConfig?.baseURL !== undefined)
        conflicting.push('baseURL');
    if (conflicting.length > 0) {
        throw new Error(`clientConfig must not contain ${conflicting.join(', ')} when bedrockMantleConfig is set; ` +
