 ...
 */
export function partsToContentBlocks(parts) {
    const blocks = [];
 ...
 */
export function contentBlocksToParts(blocks) {
    const parts = [];
 ...
 */
function _convertFilePart(file) {
    if ('bytes' in file) {
        ...
        if (fileType === 'image') {
            return new ImageBlock({ format: format, source: { bytes: decoded } });
        }
        if (fileType === 'video') {
            return new VideoBlock({ format: format, source: { bytes: decoded } });
        }
...
function _getFileType(mimeType) {
    ...
    if (lower.startsWith('video/'))
        return 'video';
    if (lower.startsWith('text/') || lower.startsWith('application/'))
        return 'document';
    return 'unknown';
}
/**
 ...
 */
function _getFormat(mimeType, fileType) {
    if (!mimeType) {
 ...
 */
function _mediaBlockToFilePart(block) {
 ...
 * @returns A2A FilePart, or undefined if the source type is unsupported
 */
function _documentBlockToFilePart(block) {
    const mimeType = toMimeType(block.format);
