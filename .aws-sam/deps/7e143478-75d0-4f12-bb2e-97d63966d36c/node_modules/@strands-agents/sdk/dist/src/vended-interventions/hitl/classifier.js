 ...
 */
export function createLlmRiskClassifier(config) {
    const systemPrompt = config?.systemPrompt ?? DEFAULT_SYSTEM_PROMPT;
    const configuredModel = config?.model;
    return async (event) => {
        const model = configuredModel ?? event.agent.model;
        if (!model) {
            throw new Error('LLM risk classifier has no model — pass `model` in `classifier: { model }`, or ensure the parent agent has a model.');
        }
