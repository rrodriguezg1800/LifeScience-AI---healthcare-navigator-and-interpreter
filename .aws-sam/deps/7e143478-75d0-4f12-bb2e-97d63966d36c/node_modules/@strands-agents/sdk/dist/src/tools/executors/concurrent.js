...
export class ConcurrentToolExecutor extends ToolExecutor {
    ...
    async *execute(options, input) {
        ...
        const takeNextStep = (index) => executions[index].generator.next().then((result) => ({ index, kind: 'next', result }), (error) => ({ index, kind: 'throw', error }));
        ...
        try {
            ...
            await Promise.allSettled(Array.from(pendingSteps.keys(), (index) => executions[index].generator.return(undefined)));
