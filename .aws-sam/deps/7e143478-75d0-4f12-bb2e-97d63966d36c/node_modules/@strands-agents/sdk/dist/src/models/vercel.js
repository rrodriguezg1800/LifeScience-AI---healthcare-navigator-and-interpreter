 ...
 * ```
 */
export class VercelModel extends Model {
    _provider;
    _config;
    /**
    ...
    async *stream(messages, options) {
        ...
        const clientToolInputStarts = new Set();
...
function* mapStreamPart(part) {
    switch (part.type) {
        ...
        case 'finish':
            ...
            break;
        case 'error':
            throw new ModelError(`Language model stream error: ${part.error instanceof Error ? part.error.message : JSON.stringify(part.error)}`, { cause: part.error });
 ...
 */
function formatUserMessage(message, prompt, toolNameMap) {
    const content = [];
    const toolResults = [];
    for (const block of message.content) {
        switch (block.type) {
 ...
 * @param prompt - The prompt array to push formatted messages into
 */
function formatAssistantMessage(message, prompt) {
    const content = [];
    for (const block of message.content) {
        switch (block.type) {
            case 'textBlock':
 ...
 */
function formatMediaBlock(block) {
    const parts = [];
    switch (block.type) {
        case 'imageBlock': {
 ...
 */
function formatToolResultOutput(status, content) {
    if (status === 'error') {
    ...
    for (const c of content) {
        switch (c.type) {
            ...
            default:
                logger.warn(`block_type=<${c.type}> | unsupported content type in vercel tool result, skipping`);
                break;
        }
    }
 ...
 */
function formatTools(toolSpecs) {
...
function formatToolChoice(toolChoice) {
    ...
    if ('tool' in toolChoice)
        return { type: 'tool', toolName: toolChoice.tool.name };
    return { type: 'auto' };
}
/**
