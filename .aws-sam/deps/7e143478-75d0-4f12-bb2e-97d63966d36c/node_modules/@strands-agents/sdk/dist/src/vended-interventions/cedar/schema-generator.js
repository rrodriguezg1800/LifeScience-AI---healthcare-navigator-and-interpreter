...
export function createSchemaGenerator(wasm, namespace) {
...
`;
    return {
        generateSchema(tools) {
            const config = JSON.stringify({ flattenNamespaces: true });
            const result = JSON.parse(wasm.generateSchema(defaultStub, JSON.stringify(tools), config));
